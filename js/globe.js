...
// OPTIMIZED: Pauses when not visible
// ========================================
...
class MinimalGlobe {
  constructor(canvasId) {
    ...
    this.dots = [];
    this.routes = [];
    this.isVisible = false; // Start paused
  ...
  setupVisibilityObserver() {
    ...
    const orbitalSystem = document.querySelector('.globe-orbital-system');
    
    if (!stepsSection) {
      // Fallback: just start animation
      this.isVisible = true;
      this.animate();
      return;
    }
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting && entry.intersectionRatio > 0.1) {
          // Section is visible - start/resume animation
          if (!this.isVisible) {
            this.isVisible = true;
            this.animate();
            
            // Resume orbital flags CSS animation
            if (orbitalSystem) {
              orbitalSystem.classList.remove('orbital-paused');
            }
          }
        } else {
          // Section not visible - pause animation
          ...
          // Pause orbital flags CSS animation
          if (orbitalSystem) {
            orbitalSystem.classList.add('orbital-paused');
          }
        }
      });
    }, {
      threshold: [0, 0.1, 0.3, 0.5],
      rootMargin: '100px 0px' // Start a bit before section comes into view
    });
  ...
  // Public method to manually pause/resume
  pause() {
    this.isVisible = false;
    if (this.animationId) {
      cancelAnimationFrame(this.animationId);
...
// Store globe instance globally for potential external control
...
document.addEventListener('DOMContentLoaded', () => {
  setTimeout(() => {
    ...
    if (canvas) {
      globeInstance = new MinimalGlobe('globe-canvas');
    }
  }, 100);
});

// Export for external use if needed
window.MinimalGlobe = MinimalGlobe;
window.getGlobeInstance = () => globeInstance;
